...
public class MergeTrees implements Serializable {
	...
	public void output(String op, String part, TreeType tree) {
		try {
			...
			int noNodes = 0;
			for(int i = 0;i < noVertices;i ++) {
				if(criticalPts[i] != CTAlgorithm.REGULAR) {
					noNodes ++;
				}
			}
			if(newVertex) {
			...
			if(newVertex) {
				if(tree == TreeType.SplitTree){
					p.println(noVertices + " " + fnVertices[sv[0]] + " " + string[CTAlgorithm.MINIMUM]);	
				}
			}
			for(int i = 0;i < noVertices;i ++) {
				if(criticalPts[sv[i]] != CTAlgorithm.REGULAR) {
					p.println(sv[i] + " " + fnVertices[sv[i]] + " " + string[criticalPts[sv[i]]]);
				}
			}
			if(newVertex) {
				if(tree != TreeType.SplitTree){
					p.println(noVertices + " " + fnVertices[sv[noVertices - 1]] + " " + string[CTAlgorithm.MAXIMUM]);
				}
			}
			
			ArrayList<Integer> arcFrom = new ArrayList<Integer>();
			ArrayList<Integer> arcTo = new ArrayList<Integer>();
			ArrayList<Integer> arcNos  = new ArrayList<Integer>();
			if(tree == TreeType.SplitTree) {
				int arcNo = 0;
				for(int i = 0;i < noVertices;i ++) {
					if((criticalPts[i] == CTAlgorithm.MAXIMUM || criticalPts[i] == CTAlgorithm.SADDLE) && i != sv[0]) {
						arcMap[i] = arcNo;
						
						int to = i;
						int from = prev[to];
						
						while(criticalPts[from] == CTAlgorithm.REGULAR) {
							arcMap[from] = arcNo;
							from = prev[from];
						}
						arcMap[from] = arcNo;
						...
						if(criticalPts[i] == CTAlgorithm.SADDLE) {
							arcFrom.add(from);
							arcTo.add(to);
							arcNos.add(arcNo);
						}
				...
				int arcNo = 0;
				for(int i = 0;i < noVertices;i ++) {
					if((criticalPts[i] == CTAlgorithm.MINIMUM || criticalPts[i] == CTAlgorithm.SADDLE) && i != sv[sv.length - 1]) {
						arcMap[i] = arcNo;
						
						int from = i;
						int to = next[from];
						
						while(criticalPts[to] == CTAlgorithm.REGULAR) {
							arcMap[to] = arcNo;
							to = next[to];
						}
						arcMap[to] = arcNo;
		...
		try {
			...
			if(newVertex) {
				if(tree == TreeType.SplitTree){
					...
					rg.nodes[in].fn = fnVertices[sv[0]];
					rg.nodes[in].type = rg.getType(string[CTAlgorithm.MINIMUM].trim());
					in ++;
				}
			}
			for(int i = 0;i < noVertices;i ++) {
				if(criticalPts[sv[i]] != CTAlgorithm.REGULAR) {
					...
					rg.nodes[in].v = sv[i];
					rg.nodeMap.put(rg.nodes[in].v, in);
					rg.nodes[in].fn = fnVertices[sv[i]];
			...
			if(newVertex) {
				if(tree != TreeType.SplitTree){
					...
					rg.nodes[in].v = noVertices;
					rg.nodeMap.put(rg.nodes[in].v, in);
					rg.nodes[in].fn = fnVertices[sv[noVertices - 1]];
					rg.nodes[in].type = rg.getType(string[CTAlgorithm.MAXIMUM].trim());
					in ++;
				}
			}
			...
			if(tree == TreeType.SplitTree) {
				...
				for(int i = 0;i < noVertices;i ++) {
					if((criticalPts[i] == CTAlgorithm.MAXIMUM || criticalPts[i] == CTAlgorithm.SADDLE) && i != globalMin) {
						arcMap[i] = arcNo;
				...
				for(int i = 0;i < noVertices;i ++) {
					if((criticalPts[i] == CTAlgorithm.MINIMUM || criticalPts[i] == CTAlgorithm.SADDLE) && i != globalMax) {
						arcMap[i] = arcNo;
...
//						p.println(from + " " + to);
	...
	static {
		...
		string[CTAlgorithm.MINIMUM] = "MINIMA";
		string[CTAlgorithm.MAXIMUM] = "MAXIMA";
		string[CTAlgorithm.SADDLE] = "SADDLE";
		string[CTAlgorithm.REGULAR] = "REGULAR";
	}
	byte [] criticalPts;
	...
	public void findSplitTree() {
		for(int i = noVertices - 1;i >= 0; i --) {
			...
			criticalPts[v] = CTAlgorithm.REGULAR;
			processVertex(v);
		}
		int in = 0;
		...
		if(criticalPts[sv[in]] == CTAlgorithm.SADDLE) {
			// add a new vertex
			newVertex = true;
		} else {
			criticalPts[sv[in]] = CTAlgorithm.MINIMUM;
		}
		newRoot = in;
	}
	...
	void processVertex(int v) {
		...
		if(set.size() == 0) {
			...
			cpMap[comp] = v;
			criticalPts[v] = CTAlgorithm.MAXIMUM;
		} else {
			if(set.size() > 1) {
				criticalPts[v] = CTAlgorithm.SADDLE;
			}
	...
	public void findJoinTree() {
		...
		if(criticalPts[sv[in]] == CTAlgorithm.SADDLE) {
			...
			criticalPts[sv[in]] = CTAlgorithm.MAXIMUM;
		}
		newRoot = in;
	}
	...
	void processVertexJ(int v) {
		...
		if(set.size() == 0) {
			...
			int comp = nodes.find(v);
			cpMap[comp] = v;
			criticalPts[v] = CTAlgorithm.MINIMUM;
		} else {
			if(set.size() > 1) {
				criticalPts[v] = CTAlgorithm.SADDLE;
			}
