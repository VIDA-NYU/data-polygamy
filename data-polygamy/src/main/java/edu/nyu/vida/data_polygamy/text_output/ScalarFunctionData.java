...
public class ScalarFunctionData {
	...
	public static void main(String[] args) throws IOException,
        ...
        try {
            ...
            formatter.printHelp("hadoop jar data-polygamy.jar " +
                    "edu.nyu.vida.data_polygamy.text_output.ScalarFunctionData", options, true);
            System.exit(0);
        }
        ...
        for (String dataset: shortDataset) {
            ...
            String aggregateTextOutputFileName = s3bucket + FrameworkUtils.aggregatesTextDir + "/" + dataset + "/";
            
            if (removeExistingFiles) {
                FrameworkUtils.removeFile(aggregateTextOutputFileName, s3conf, s3);
            }
            
            if (!FrameworkUtils.fileExists(aggregateTextOutputFileName, s3conf, s3)) {
        ...
        job.setMapperClass(ScalarFunctionDataMapper.class);
        ...
        FileInputFormat.setInputPaths(job, aggregateDatasets.substring(0,
                aggregateDatasets.length()-1));
        FileOutputFormat.setOutputPath(job, new Path(aggregateOutputDir));
   
        job.setJarByClass(ScalarFunctionData.class);
